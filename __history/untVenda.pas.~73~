...
begin
  ...
  // remove o segundo "begin" que estava aqui — continua direto
  with dmconexoes do
  begin
    qrVendas.close;
    qrVendas.sql.clear;
    ...
    begin
      qrItensVenda.Insert;
      qrItensVenda.FieldByName('CodItem').value      := fdProduto.FieldByName('CodProd').AsString;
      ...
      qrItensVenda.FieldByName('ValorTotal').value := fdProduto.FieldByName('ValorTotal').AsCurrency;
      qrItensVenda.FieldByName('Quantidade').AsInteger  := fdProduto.FieldByName('Quantidade').AsInteger;
      qrItensVenda.FieldByName('ValorProdUni').value := fdProduto.FieldByName('ValorUni').AsCurrency;
...
begin
  ...
  begin
    ...
    begin
      ...
      qrCliente.sql.clear;
      qrCliente.sql.Add
        ('SELECT * FROM [LojaNova].[dbo].[Cliente] where codcli = :pcod');
